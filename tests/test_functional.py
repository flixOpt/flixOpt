...
class BaseTest(unittest.TestCase):
        ...
        self.flow_system.add_elements(
            fx.Sink(
                ...
                sink=fx.Flow(
                    ...
                    fixed_relative_profile=data.thermal_demand,
                    size=1,
                ),
            ),
            fx.Source(
                label='Gastarif',
                source=fx.Flow(label='Gas', bus=self.get_element('Gas'), effects_per_flow_hour=1),
            ),
        )
    ...
    def solver(self):
        ...
        return fx.solvers.HighsSolver(
            mip_gap=self.mip_gap,
            time_limit_seconds=3600,
            solver_output_to_console=False,
        )
...
class TestOnOff(BaseTest):
    ...
    def test_on_total_bounds(self):
        ...
        self.get_element('Wärmelast').sink.fixed_relative_profile = [
            0,
            10,
            20,
            0,
