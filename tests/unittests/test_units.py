...
def run_model():
    import numpy as np
    import datetime
    from flixOpt.flixStructure import cFlow, cBus, cEnergySystem, cEffectType, cCalculation
    from flixOpt.flixComps import cKWK, cKessel, cStorage, cBaseLinearTransformer, cSink, cSource
    from flixOpt.flixBasicsPublic import cInvestArgs
    ...
    CO2 = cEffectType('CO2', 'kg', 'CO2_e-Emissionen',
                      ...
                      specificShareToOtherEffects_invest={costs: 0.2})
    PE = cEffectType('PE', 'kWh_PE', 'Primärenergie', max_Sum=3.5e3,
                     specificShareToOtherEffects_operation={CO2: 2},
                     specificShareToOtherEffects_invest={CO2: 2})
    ...
    invest_Speicher = cInvestArgs(fixCosts=0,  # no fix costs
                                  ...
                                  specificCosts={costs: 0.01, CO2: 0.01, PE: 0.005},  # €/kWh; kg_CO2/kWh
    ...
    aGasTarif = cSource('Gastarif',
                        source=cFlow('Q_Gas',
                                     ...
                                     costsPerFlowHour={costs: 0.04, CO2: 0.3, PE: 0.1}), )
    ...
    calc1 = flixPost.flix_results(aCalc.nameOfCalc, comp_colors=comp_colors)
    return calc1
...
class TestFunctions(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            raise unittest.SkipTest(f"Initial setup of Test failed with exception: {e}")


    def test_all_shares_factors(self):
        shares = self.calc1._get_effect_share_factors()
        expected_result = {'CO2': {'costs': 0.2}, 'PE': {'CO2': 2, 'costs': 0.4}}
        self.assertEqual(shares, expected_result, "The results do not match the expected values.")

    def test_results_wo_shares_operation(self):
        results_wo_shares = {}
        for comp in self.calc1.comps:
            results_wo_shares[comp] = self.calc1.get_effect_result_without_shares("PE", comp, "operation")

        expected_result = {'Kessel': 0, 'Waermelast': 0, 'Gastarif': 97.728828, 'Einspeisung': 0, 'BHKW2': 0, 'Speicher': 0}

        self.assertEqual(results_wo_shares, expected_result, "The results do not match the expected values.")

    def test_results_wo_shares_invest(self):
        results_wo_shares = {}
        for comp in self.calc1.comps:
            results_wo_shares[comp] = self.calc1.get_effect_result_without_shares("PE", comp, "invest")

        expected_result = {'Kessel': 100, 'Waermelast': 0, 'Gastarif': 0, 'Einspeisung': 0, 'BHKW2': 0, 'Speicher': 100.5}

        self.assertEqual(results_wo_shares, expected_result, "The results do not match the expected values.")

    def test_results_with_shares_operation(self):

        results_wo_shares = self.calc1.get_effect_result("costs", "Gastarif", "operation")
        expected_result_pe = 97.728828
        expected_result_co2 = expected_result_pe * 2 + 293.18648
        expected_result = expected_result_co2 * 0.2 + 39.091531

        self.assertAlmostEqual(results_wo_shares, expected_result, 3, "The results do not match the expected values.")

    #TODO: Problem with different shares of invest and operation!!
