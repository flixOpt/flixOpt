...
class TestSimple(BaseTest):
    ...
    def model(self):
        ...
        aKWK = CHP('CHP_unit', eta_th=0.5, eta_el=0.4, P_el=Flow('P_el', bus=Strom, nominal_val=60, min_rel=5 / 60),
                   Q_th=Flow('Q_th', bus=Fernwaerme), Q_fu=Flow('Q_fu', bus=Gas))
        aSpeicher = Storage('Speicher', inFlow=Flow('Q_th_load', bus=Fernwaerme, nominal_val=1e4),
                            outFlow=Flow('Q_th_unload', bus=Fernwaerme, nominal_val=1e4), capacity_inFlowHours=30,
                            ...
                            investArgs=InvestArgs(fixCosts=20, investmentSize_is_fixed=True,
...
class TestComplex(BaseTest):
    ...
    def basic_model(self):
        ...
        invest_Gaskessel = InvestArgs(fixCosts=1000, investmentSize_is_fixed=True, investment_is_optional=False, specificCosts={costs: 10, PE: 2})
        aGaskessel = Boiler('Kessel', eta=0.5, costsPerRunningHour={costs: 0, CO2: 1000},
                            Q_th=Flow('Q_th', bus=Fernwaerme, nominal_val=50, loadFactor_max=1.0, loadFactor_min=0.1, min_rel=5 / 50, max_rel=1, onHoursSum_min=0, onHoursSum_max=1000, onHours_max=10, offHours_max=10, switchOnCosts=0.01, switchOn_maxNr=1000, valuesBeforeBegin=[50], investArgs=invest_Gaskessel, sumFlowHours_max=1e6),
        ...
        invest_Speicher = InvestArgs(fixCosts=0, investmentSize_is_fixed=False, costsInInvestsizeSegments=costsInvestsizeSegments, investment_is_optional=False, specificCosts={costs: 0.01, CO2: 0.01}, min_investmentSize=0, max_investmentSize=1000)
