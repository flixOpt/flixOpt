...
class SystemModel(BaseModel):
    ...
    def registerMEandMod(self, aModelingElement, aMod):
        # allocation ME -> element_model
        self.ME_mod[aModelingElement] = aMod  # aktuelles element_model hier speichern
    ...
    def solve(self, gapFrac=0.02, timelimit=3600, solver='cbc', displaySolverOutput=True, excessThreshold=0.1,
        ...
        **kwargs : TYPE
        ...
        for aEffect in self.es.globalComp.listOfEffectTypes:
            ...
            print('  operation: ' + str(aEffect.operation.element_model.var_sum.getResult()))
            print('  invest   : ' + str(aEffect.invest.element_model.var_sum.getResult()))
            print('  sum      : ' + str(aEffect.all.element_model.var_sum.getResult()))

        print('SUM              : ' + '...todo...')
        print('penaltyCosts     : ' + str(self.es.globalComp.penalty.element_model.var_sum.getResult()))
        print('––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––')
        ...
        for aBus in self.es.setOfBuses:
            if aBus.withExcess:
                if any(self.results[aBus.label]['excessIn'] > 1e-6) or any(
                    ...
                    # if penalties exist
        if self.es.globalComp.penalty.element_model.var_sum.getResult() > 10:
        ...
        def _getMainResultsAsStr():
            ...
            for aEffect in self.es.globalComp.listOfEffectTypes:
                ...
                aDict['operation'] = str(aEffect.operation.element_model.var_sum.getResult())
                aDict['invest'] = str(aEffect.invest.element_model.var_sum.getResult())
                aDict['sum'] = str(aEffect.all.element_model.var_sum.getResult())
            main_results_str['penaltyCosts'] = str(self.es.globalComp.penalty.element_model.var_sum.getResult())
            ...
            for aInvestFeature in self.es.allInvestFeatures:
                investValue = aInvestFeature.element_model.var_investmentSize.getResult()
...
class Element(cArgsClass):
    ...
    def __init__(self, label: str, **kwargs):
        ...
        self.element_model: Optional[ElementModel] = None  # hier kommen alle Glg und Vars rein
    ...
    def _activateModBox_ForMeOnly(self, modBox) -> None:
        ...
        self.element_model = modBox.getModOfME(self)
    ...
    def createNewModAndActivateModBox(self, modBox) -> None:
        # print('new element_model for ' + self.label)
        # subElemente ebenso:
        aME: Element
        for aME in self.subElements:
            aME.createNewModAndActivateModBox(modBox)  # rekursiv!

        # create element_model:
        aMod = ElementModel(self)
        # register element_model:
    ...
    def declareVarsAndEqs(self, modBox) -> None:
        ...
        #   #   aFeature.declareVarsAndEqs(system_model)
        pass

    # def doModeling(self,system_model,timeIndexe):
    #   # for aFeature in self.features:
    #   aFeature.doModeling(system_model, timeIndexe)
    ...
    def getResults(self) -> Tuple[Dict, Dict]:
        ...
        aVar: Variable
        for aVar in self.element_model.variables:
            # print(aVar.label)
    ...
    def var(self, label):
        self.element_model.getVar(label)

    def eq(self, label):
        self.element_model.getEq(label)
    ...
    def getEqsAsStr(self):
        ...
        if not (subs == {}):
            eqsAsStr = {}
            eqsAsStr['_self'] = self.element_model.getEqsAsStr()  # zuerst eigene ...
            eqsAsStr.update(subs)  # ... dann sub-Eqs
        # sonst liste:
        else:
            eqsAsStr = self.element_model.getEqsAsStr()

        return eqsAsStr

    def getVarsAsStr(self) -> List:
        aList = []
        aList += self.element_model.getVarsAsStr()
        for aSubElement in self.subElements:
    ...
    def getEqsVarsOverview(self) -> Dict:
        aDict = {}
        aDict['no eqs'] = len(self.element_model.eqs)
        aDict['no eqs single'] = sum([eq.nrOfSingleEquations for eq in self.element_model.eqs])
        aDict['no inEqs'] = len(self.element_model.ineqs)
        aDict['no inEqs single'] = sum([ineq.nrOfSingleEquations for ineq in self.element_model.ineqs])
        aDict['no vars'] = len(self.element_model.variables)
        aDict['no vars single'] = sum([var.len for var in self.element_model.variables])
        return aDict
...
class Effect(Element):
    ...
    def doModeling(self, modBox, timeIndexe) -> None:
        ...
        self.all.addVariableShare('operation', self, self.operation.element_model.var_sum, 1, 1)
        self.all.addVariableShare('invest', self, self.invest.element_model.var_sum, 1, 1)
...
class Component(Element):
    ...
    def __init__(self, label: str, on_valuesBeforeBegin:Optional[List[Skalar]] = None,
        ...
        **kwargs : TYPE
        ...
        # self.on_valuesBefore = 1 * (self.featureOwner.valuesBeforeBegin >= np.maximum(system_model.epsilon,self.flowMin)) für alle Flows!
    ...
    def declareVarsAndEqs(self, modBox) -> None:
        ...
        self.element_model.var_on = self.featureOn.getVar_on()  # mit None belegt, falls nicht notwendig
        self.element_model.var_switchOn, self.element_model.var_switchOff = self.featureOn.getVars_switchOnOff()  # mit None belegt, falls nicht notwendig
...
class Global(Element):
    ...
    #  eq_objective = Equation('objective',self,system_model,'objective')
    ...
    def doModeling(self, modBox, timeIndexe) -> None:
        ...
        for effectType in self.listOfEffectTypes:
            ...
            for effectTypeOfShare, specShare_TS in effectType.specificShareToOtherEffects_operation.items():
                ...
                shareSum_op.addVariableShare(nameOfShare, shareHolder, effectType.operation.element_model.var_sum_TS,
                                             specShare_TS, 1)
            ...
            for effectTypeOfShare, specShare in effectType.specificShareToOtherEffects_invest.items():
                ...
                shareHolder = effectType
                shareSum_inv.addVariableShare(nameOfShare, shareHolder, effectType.invest.element_model.var_sum, specShare, 1)

        # ####### target function  ###########
        # Strafkosten immer:
        self.objective.addSummand(self.penalty.element_model.var_sum, 1)
        ...
        self.objective.addSummand(objectiveEffect.operation.element_model.var_sum, 1)
        self.objective.addSummand(objectiveEffect.invest.element_model.var_sum, 1)
...
class Bus(Component):  # sollte das wirklich geerbt werden oder eher nur Element???
    ...
    def doModeling(self, modBox, timeIndexe) -> None:
        ...
        for aFlow in self.inputs:
            eq_busbalance.addSummand(aFlow.element_model.var_val, 1)
        for aFlow in self.outputs:
            eq_busbalance.addSummand(aFlow.element_model.var_val, -1)
    ...
    def addShareToGlobals(self, globalComp, modBox) -> None:
        ...
        if self.withExcess:
            ...
            globalComp.penalty.addVariableShare('excessCostsPerFlowHour', self, self.excessOut,
                                                self.excessCostsPerFlowHour, modBox.dtInHours)
            # globalComp.penaltyCosts_eq.addSummand(self.excessIn , np.multiply(self.excessCostsPerFlowHour, system_model.dtInHours))
            # globalComp.penaltyCosts_eq.addSummand(self.excessOut, np.multiply(self.excessCostsPerFlowHour, system_model.dtInHours))
...
class IO():
    ...
    # input wären wie Flow,aber statt bus : connectsTo -> hier andere IO oder aber Bus (dort keine IO, weil nicht notwendig)
...
class Flow(Element):
    ...
    def __init__(self, label,
        ...
        on_valuesBeforeBegin = 1 * (
                    self.valuesBeforeBegin >= 0.0001)  # TODO: besser wäre system_model.epsilon, aber hier noch nicht bekannt!)
    ...
    def declareVarsAndEqs(self, modBox: SystemModel) -> None:
        ...
        def getMinMaxOfDefiningVar():
            ...
            if self.val_rel is not None:
                ...
                if self.featureOn.useOn:
                    ...
                    lb = self.min_rel.active_data * self.nominal_val  # immer an
                ub = self.max_rel.active_data * self.nominal_val
                fix_value = None
        ...
        self.element_model.var_val = VariableTS('val', modBox.nrOfTimeSteps, self, modBox, min=lb, max=ub, value=fix_value)
        self.element_model.var_sumFlowHours = Variable('sumFlowHours', 1, self, modBox, min=self.sumFlowHours_min,
        ...
        if self.featureInvest is not None:
            self.featureInvest.setDefiningVar(self.element_model.var_val, self.element_model.var_on)
    ...
    def doModeling(self, modBox: SystemModel, timeIndexe) -> None:
        ...
        eq_sumFlowHours.addSummandSumOf(self.element_model.var_val, modBox.dtInHours)
        eq_sumFlowHours.addSummand(self.element_model.var_sumFlowHours, -1)
        ...
        if self.loadFactor_max is not None:
            ...
            eq_flowHoursPerInvestsize_Max.addSummand(self.element_model.var_sumFlowHours, 1)
            if self.featureInvest is not None:
                eq_flowHoursPerInvestsize_Max.addSummand(self.featureInvest.element_model.var_investmentSize,
        ...
        if self.loadFactor_min is not None:
            ...
            eq_flowHoursPerInvestsize_Min.addSummand(self.element_model.var_sumFlowHours, -1)
            if self.featureInvest is not None:
                eq_flowHoursPerInvestsize_Min.addSummand(self.featureInvest.element_model.var_investmentSize,
        ...
        if self.positive_gradient == None :                    
          if system_model.modType == 'pyomo':
            def positive_gradient_rule(t):
              if t == 0:
                return (self.element_model.var_val[t] - self.val_initial) / system_model.dtInHours[t] <= self.positive_gradient[t] #             
              else: 
                return (self.element_model.var_val[t] - self.element_model.var_val[t-1])    / system_model.dtInHours[t] <= self.positive_gradient[t] #
            ...
            if (self.val_initial == None) & (start == 0):
              ...
              self.positive_gradient_constr =  Constraint(system_model.timestepsOfRun,rule = positive_gradient_rule)   # timestepsOfRun = [start:end]
              # raise error();
            modbox.registerPyComp(self.positive_gradient_constr, self.label + '_positive_gradient_constr')
          elif system_model.modType == 'vcxpy':
            raise Exception('not defined for modtype ' + system_model.modType)
          else:
            raise Exception('not defined for modtype ' + system_model.modType)'''
    ...
    def addShareToGlobals(self, globalComp: Global, modBox) -> None:
        ...
        if self.costsPerFlowHour is not None:
            ...
            # variable_costs          = cVector(self.element_model.var_val, np.multiply(self.costsPerFlowHour, system_model.dtInHours))
            # globalComp.costsOfOperating_eq.addSummand(self.element_model.var_val, np.multiply(self.costsPerFlowHour.active_data, system_model.dtInHours)) # np.multiply = elementweise Multiplikation
            shareHolder = self
            globalComp.addShareToOperation('costsPerFlowHour', shareHolder, self.element_model.var_val, self.costsPerFlowHour,
                                           modBox.dtInHours)
...
class System:
    ...
    def __init__(self, timeSeries, dt_last=None):
        ...
        self.system_model: SystemModel = None  # later activated
    ...
    def doModelingOfElements(self) -> SystemModel:
        ...
        timeIndexe = range(len(self.system_model.esTimeIndexe))

        # globale Modellierung zuerst, damit andere darauf zugreifen können:
        self.globalComp.declareVarsAndEqs(self.system_model)  # globale Funktionen erstellen!
        self.globalComp.doModeling(self.system_model, timeIndexe)  # globale Funktionen erstellen!
        ...
        for aComp in self.listOfComponents:
            ...
            aComp.declareVarsAndEqsOfFlows(self.system_model)
            aComp.declareVarsAndEqs(self.system_model)

            aComp.doModelingOfFlows(self.system_model, timeIndexe)
            aComp.doModeling(self.system_model, timeIndexe)

            aComp.addShareToGlobalsOfFlows(self.globalComp, self.system_model)
            aComp.addShareToGlobals(self.globalComp, self.system_model)

        # Bus-Modellierung (# inklusive subMEs!)
        aBus: Bus
        for aBus in self.setOfBuses:
            log.debug('model ' + aBus.label + '...')
            aBus.declareVarsAndEqs(self.system_model)
            aBus.doModeling(self.system_model, timeIndexe)
            aBus.addShareToGlobals(self.globalComp, self.system_model)

        # weitere übergeordnete Modellierungen:
        for aME in self.setOfOtherMEs:
            aME.declareVarsAndEqs(self.system_model)
            aME.doModeling(self.system_model, timeIndexe)
            aME.addShareToGlobals(self.globalComp, self.system_model)

            # transform to Math:
        self.system_model.transform2MathModel()
    ...
    def activateModBox(self, aModBox:SystemModel) -> None:
        ...
        # Wenn noch nicht gebaut, dann einmalig ME.element_model bauen:
        if aModBox.ME_mod == {}:
            log.debug('create element_model-Vars for MEs of EnergySystem')
            for aME in self.allMEsOfFirstLayer:
                ...
                # element_model bauen und in system_model registrieren.
                aME.createNewModAndActivateModBox(self.system_model)  # inkl. subMEs
        else:
            # nur Aktivieren:
            for aME in allMEsOfFirstLayer:  # TODO: Is This a BUG?
                aME.activateModbox(aModBox)  # inkl. subMEs

    # ! nur nach Solve aufrufen, nicht später nochmal nach activating system_model (da evtl stimmen Referenzen nicht mehr unbedingt!)
    def getResultsAfterSolve(self) -> Tuple[Dict, Dict]:
        ...
        aTime = {}
        results['time'] = aTime
        aTime['timeSeriesWithEnd'] = self.system_model.timeSeriesWithEnd
        aTime['timeSeries'] = self.system_model.timeSeries
        aTime['dtInHours'] = self.system_model.dtInHours
        aTime['dtInHours_tot'] = self.system_model.dtInHours_tot
    ...
    def getVarsAsStr(self, structured=True) -> Union[List, Dict]:
        ...
        if not structured:
            aList = []
            for aVar in self.system_model.variables:
                aList.append(aVar.getStrDescription())
            return aList
...
class Calculation:
    ...
    def __init__(self, label, es: System, modType, chosenEsTimeIndexe=None, pathForSaving='results', ):
        ...
        self.segmentModBoxList = []  # system_model list
    ...
    def doSegmentedModelingAndSolving(self, solverProps, segmentLen, nrOfUsedSteps, namePrefix='', nameSuffix='',
        ...
        for i in range(nrOfSimSegments):
            ...
            # system_model in Energiesystem aktivieren:
    ...
    def doAggregatedModeling(self, periodLengthInHours, noTypicalPeriods,
        ...
        import pandas as pd
        # seriesDict = {i : self.TSlistForAggregation[i].active_data_raw_vector for i in range(len(self.TSlistForAggregation))}
