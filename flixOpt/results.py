...
class CalculationResults:
    ...
    def _construct_bus_results(self):
        ...
        for bus_label in bus_results.keys():
            infos, results = (
                bus_infos.get(bus_label, {}),
                bus_results.get(bus_label, {}),
            )
            inputs = [
                flow
    ...
    def visualize_network(
        ...
        return plotting.visualize_network(
            self.calculation_infos['Network']['Nodes'],
            self.calculation_infos['Network']['Edges'],
            path,
            controls,
            show,
        )
...
class BusResults(ElementResults):
    def __init__(
        ...
        infos: Dict,
        results: Dict,
        inputs: List[FlowResults],
        outputs: List[FlowResults],
    ):
        super().__init__(infos, results)
        self.inputs = inputs
...
def extract_single_result(
    results_data: dict[str, Dict[str, Union[int, float, np.ndarray, dict]]],
    keys: List[str],
...
if __name__ == '__main__':
    results = CalculationResults(
        'Sim1',
        '/Users/felix/Documents/Dokumente - eigene/Neuer Ordner/flixOpt-Fork/examples/Ex02_complex/results',
    )
    ...
    plotting.heat_map_plotly(
        plotting.heat_map_data_from_df(
            pd.DataFrame(
                results.component_results['Speicher'].variables['charge_state'],
                index=results.time_with_end,
            ),
            periods='D',
    ...
    fig = plotting.with_plotly(
        pd.DataFrame(
            extract_results(results.all_results['Components'], ['OnOff', 'on']),
            index=results.time,
        ),
        mode='bar',
    )
