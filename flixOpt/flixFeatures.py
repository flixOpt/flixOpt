...
class cFeatureLinearSegmentVars(cFeature):
    ...
    def doModeling(self, modBox: SystemModel, timeIndexe):
        ...
        for aSegment in self.listOfSegments:
            self.eq_IcanOnlyBeInOneSegment.addSummand(aSegment.element_model.var_onSeg, 1);
        ...
        for aSegment in self.listOfSegments:
            ...
            eq_Lambda_onSeg.addSummand(aSegment.element_model.var_onSeg, -1);
            eq_Lambda_onSeg.addSummand(aSegment.element_model.var_lambda1, 1);
            eq_Lambda_onSeg.addSummand(aSegment.element_model.var_lambda2, 1);
        ...
        for aVar in self.segmentsOfVars.keys():
            # aVar = aFlow.element_model.var_val
            ...
            for aSegment in self.listOfSegments:
                ...
                eqLambda.addSummand(aSegment.element_model.var_lambda1,
                                    samplePoint1)  # Spalte 1 (Faktor kann hier Skalar sein oder Vektor)
                eqLambda.addSummand(aSegment.element_model.var_lambda2,
...
class cFeatureLinearSegmentSet(cFeatureLinearSegmentVars):
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        for flow in self.segmentsOfFlows_TS:
            segmentsOfVars[flow.element_model.var_val] = self.segmentsOfFlows_TS[flow]

        checkListOfVars = []
        for flow in self.checkListOfFlows:
            checkListOfVars.append(flow.element_model.var_val)
...
class cSegment(cFeature):
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        self.element_model.var_onSeg = VariableTS('onSeg_' + str(self.index), aLen, self, modBox,
                                                  isBinary=True)  # Binär-Variable
        self.element_model.var_lambda1 = VariableTS('lambda1_' + str(self.index), aLen, self, modBox, min=0,
                                                    max=1)  # Wertebereich 0..1
        self.element_model.var_lambda2 = VariableTS('lambda2_' + str(self.index), aLen, self, modBox, min=0,
                                                    max=1)  # Wertebereich 0..1
...
class cFeatureAvoidFlowsAtOnce(cFeature):
    ...
    def doModeling(self, modBox, timeIndexe):
        ...
        for aFlow in self.flows:
            # + flow_i.on(t):
            if aFlow.element_model.var_on is not None:
                self.eq_flowLock.addSummand(aFlow.element_model.var_on, 1)
                # + flow_i.val(t)/flow_i.max
            else:  # nur bei "new"
                assert aFlow.min >= 0, 'cFeatureAvoidFlowsAtOnce(): typ "new" geht nur für Flows mit min >= 0!'
                self.eq_flowLock.addSummand(aFlow.element_model.var_val, 1 / aFlow.max)
...
class cFeatureOn(cFeature):
    ...
    # #   self.flows  = flows
    # #   self.element_model.var_on = None
    ...
    def getVar_on(self):
        return self.element_model.var_on

    def getVars_switchOnOff(self):
        return self.element_model.var_switchOn, self.element_model.var_switchOff
    ...
    #   # # Check:
    #   # if (hasattr(self.featureOwner, 'var_on')) and (self.featureOwner.element_model.var_on == None) :
    #   #   self.featureOwner.element_model.var_on = self.element_model.var_on
    #   # else :
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        if self.useOn:
            # Before-Variable:
            self.element_model.var_on = VariableTS('on', modBox.nrOfTimeSteps, self.owner, modBox, isBinary=True)
            self.element_model.var_on.activateBeforeValues(esBeforeValue=self.on_valuesBeforeBegin[0],
                                                           beforeValueIsStartValue=False)
            self.element_model.var_onHoursSum = Variable('onHoursSum', 1, self.owner, modBox, min=self.onHoursSum_min,
                                                         max=self.onHoursSum_max)  # wenn max/min = None, dann bleibt das frei

        else:
            self.element_model.var_on = None
            self.element_model.var_onHoursSum = None

        if self.useOff:
            # off-Var is needed:
            self.element_model.var_off = VariableTS('off', modBox.nrOfTimeSteps, self.owner, modBox, isBinary=True)
        ...
        if self.useOnHours:
            ...
            self.element_model.var_onHours = VariableTS('onHours', modBox.nrOfTimeSteps, self.owner, modBox,
                                                        min=0, max=aMax)  # min separat
        # offHours:
        if self.useOffHours:
            aMax = None if (self.offHours_max is None) else self.offHours_max.active_data
            self.element_model.var_offHours = VariableTS('offHours', modBox.nrOfTimeSteps, self.owner, modBox,
                                                         min=0, max=aMax)  # min separat

        # Var SwitchOn
        if self.useSwitchOn:
            self.element_model.var_switchOn = VariableTS('switchOn', modBox.nrOfTimeSteps, self.owner, modBox, isBinary=True)
            self.element_model.var_switchOff = VariableTS('switchOff', modBox.nrOfTimeSteps, self.owner, modBox, isBinary=True)
            self.element_model.var_nrSwitchOn = Variable('nrSwitchOn', 1, self.owner, modBox,
                                                         max=self.switchOn_maxNr)  # wenn max/min = None, dann bleibt das frei
        else:
            self.element_model.var_switchOn = None
            self.element_model.var_switchOff = None
            self.element_model.var_nrSwitchOn = None
    ...
    def doModeling(self, modBox, timeIndexe):
        ...
        if self.useOnHours:
            cFeatureOn.__addConstraintsForOnTimeOfBinary(
                self.element_model.var_onHours, self.element_model.var_on, self.onHours_min,
                eqsOwner, modBox, timeIndexe)
        if self.useOffHours:
            cFeatureOn.__addConstraintsForOnTimeOfBinary(
                self.element_model.var_offHours, self.element_model.var_off, self.offHours_min,
                eqsOwner, modBox, timeIndexe)
    ...
    def __addConstraintsForOn(self, eqsOwner, flowsDefiningOn, modBox, timeIndexe):
        ...
        if nrOfFlows == 1:
            ...
            eq1.addSummand(aFlow.element_model.var_val, -1, timeIndexe)
            ...
            eq1.addSummand(self.element_model.var_on, 1 * np.maximum(modBox.epsilon, min_val),
        ...
        # Bei mehreren Leistungsvariablen:
            ...
            for aFlow in flowsDefiningOn:
                eq1.addSummand(aFlow.element_model.var_val, -1, timeIndexe)
            eq1.addSummand(self.element_model.var_on, 1 * modBox.epsilon,
                           timeIndexe)  # % aLeistungsVariableMin kann hier Skalar oder Zeitreihe sein!
        ...
        sumOfFlowMax = 0
        for aFlow in flowsDefiningOn:
            eq2.addSummand(aFlow.element_model.var_val, 1 / nrOfFlows, timeIndexe)
            # wenn variabler Nennwert:
            if aFlow.nominal_val is None:
        ...
        eq2.addSummand(self.element_model.var_on, - sumOfFlowMax / nrOfFlows, timeIndexe)  #
    ...
    def __addConstraintsForOff(self, eqsOwner, modBox, timeIndexe):
        ...
        eq_var_off.addSummand(self.element_model.var_off, 1)
        eq_var_off.addSummand(self.element_model.var_on, 1)
    ...
    def __addConstraintsForSwitchOnSwitchOff(self, eqsOwner, modBox, timeIndexe):
        ...
        eq_SwitchOnOff_andOn.addSummand(self.element_model.var_switchOn, 1, timeIndexe[1:])  # SwitchOn(t)
        eq_SwitchOnOff_andOn.addSummand(self.element_model.var_switchOff, -1, timeIndexe[1:])  # SwitchOff(t)
        eq_SwitchOnOff_andOn.addSummand(self.element_model.var_on, -1, timeIndexe[1:])  # On(t)
        eq_SwitchOnOff_andOn.addSummand(self.element_model.var_on, +1, timeIndexe[0:-1])  # On(t-1)
        ...
        eq_SwitchOnOffAtFirstTime.addSummand(self.element_model.var_switchOn, 1, firstIndex)
        eq_SwitchOnOffAtFirstTime.addSummand(self.element_model.var_switchOff, -1, firstIndex)
        eq_SwitchOnOffAtFirstTime.addSummand(self.element_model.var_on, -1, firstIndex)
        ...
        eq_SwitchOnOffAtFirstTime.addRightSide(
            -self.element_model.var_on.beforeVal())  # letztes Element der Before-Werte nutzen,  Anmerkung: wäre besser auf lhs aufgehoben

        ## Entweder SwitchOff oder SwitchOn
        # eq: SwitchOn(t) + SwitchOff(t) <= 1 

        ineq = Equation('SwitchOnOrSwitchOff', eqsOwner, modBox, eqType='ineq')
        ineq.addSummand(self.element_model.var_switchOn, 1)
        ineq.addSummand(self.element_model.var_switchOff, 1)
        ineq.addRightSide(1)
        ...
        eq_NrSwitchOn.addSummand(self.element_model.var_nrSwitchOn, 1)
        eq_NrSwitchOn.addSummandSumOf(self.element_model.var_switchOn, -1)
    ...
    def addShareToGlobals(self, globalComp, modBox):
        ...
        if self.switchOnCosts is not None:  # and any(self.switchOnCosts.active_data != 0):
            globalComp.addShareToOperation('switchOnCosts', shareHolder, self.element_model.var_switchOn, self.switchOnCosts, 1)
        # Betriebskosten:
        if self.costsPerRunningHour is not None:  # and any(self.costsPerRunningHour):
            globalComp.addShareToOperation('costsPerRunningHour', shareHolder, self.element_model.var_on,
                                           self.costsPerRunningHour, modBox.dtInHours)
            # globalComp.costsOfOperating_eq.addSummand(self.element_model.var_on, np.multiply(self.costsPerRunningHour.active_data, system_model.dtInHours))# np.multiply = elementweise Multiplikation
...
class cFeature_ShareSum(cFeature):  # (ME = ModelingElement)
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        if self.sharesAreTS:
            ...
            self.element_model.var_sum_TS = VariableTS('sum_TS', modBox.nrOfTimeSteps, self, modBox, min = lb_TS, max = ub_TS)  # TS

        # Variable für Summe (Skalar-Summe):
        self.element_model.var_sum = Variable('sum', 1, self, modBox, min=self.minOfSum, max=self.maxOfSum)  # Skalar
    ...
    def doModeling(self, modBox, timeIndexe):
        ...
        if self.sharesAreTS:
            ...
            self.eq_sum_TS.addSummand(self.element_model.var_sum_TS, -1)
            # eq: sum = sum(sum_TS(t)) # skalar
            self.eq_sum.addSummandSumOf(self.element_model.var_sum_TS, 1)
            self.eq_sum.addSummand(self.element_model.var_sum, -1)
        else:
...
class cFeatureInvest(cFeature):
    ...
    # -> definingVar         : z.B. flow.element_model.var_val
    ...
    def declareVarsAndEqs(self, modBox):
        ...
        if lb == ub:
            # fix:
            self.element_model.var_investmentSize = Variable(self.nameOfInvestmentSize, 1, self, modBox, value=lb)
        else:
            # Bereich:
            self.element_model.var_investmentSize = Variable(self.nameOfInvestmentSize, 1, self, modBox, min=lb, max=ub)

        # b) var_isInvested:
        if self.args.investment_is_optional:
            self.element_model.var_isInvested = Variable('isInvested', 1, self, modBox, isBinary=True)
    ...
    def _defineCostSegments(self, modBox: SystemModel):
        ...
        segmentsOfVars = {self.element_model.var_investmentSize: investSizeSegs}  # i.e. {var_investSize: [0,5, 5,20]}

        ## 1.b) je Effekt -> new Variable und zugehörige Segmente ##
        self.element_model.var_list_investCosts_segmented = []
        ...
        ## 2. on_var: ##
        if self.args.investment_is_optional:
            var_isInvested = self.element_model.var_isInvested
        else:
            var_isInvested = None
    ...
    def __create_var_segmentedInvestCost(self, aEffect, modBox):
        ...
        self.element_model.var_list_investCosts_segmented.append(var_investForEffect)
    ...
    def _add_fixEq_of_definingVar_with_var_investmentSize(self, modBox):
        ...
        self.eq_fix_via_investmentSize.addSummand(self.element_model.var_investmentSize, np.multiply(-1, self.val_rel.active_data))
    ...
    def _add_max_min_of_definingVar_with_var_investmentSize(self, modBox):
        ...
        # TODO: Changed by FB
        # self.eq_max_via_investmentSize.addSummand(self.element_model.var_investmentSize, np.multiply(-1, self.max_rel.active_data))
        self.eq_max_via_investmentSize.addSummand(self.element_model.var_investmentSize, np.multiply(-1, self.max_rel.data))
        # TODO: Changed by FB
        ...
        if self._existOn:
            ...
            if not self.args.investmentSize_is_fixed:
                ...
                self.eq_min_via_investmentSize.addSummand(self.element_model.var_investmentSize, self.min_rel.active_data)
    ...
    def _add_defining_var_isInvested(self, modBox):
        ...
        if self.args.investmentSize_is_fixed:
            ...
            self.eq_isInvested_1.addSummand(self.element_model.var_investmentSize, -1)
            self.eq_isInvested_1.addSummand(self.element_model.var_isInvested, self.fixedInvestmentSize)
            ...
            self.eq_isInvested_1.addSummand(self.element_model.var_investmentSize, 1)
            self.eq_isInvested_1.addSummand(self.element_model.var_isInvested,
            ...
            self.eq_isInvested_2.addSummand(self.element_model.var_investmentSize, -1)
            self.eq_isInvested_2.addSummand(self.element_model.var_isInvested, max(modBox.epsilon, self.args.min_investmentSize))
    ...
    def addShareToGlobals(self, globalComp, modBox):
        ...
        if not (self.args.fixCosts is None) and self.args.fixCosts != 0:
            if self.args.investment_is_optional:
                ...
                globalComp.addShareToInvest('fixCosts', self.owner, self.element_model.var_isInvested, self.args.fixCosts, 1)
            else:
                # share: + fixCosts
        ...
        if not (self.args.divestCosts is None) and self.args.divestCost != 0:
            if self.args.investment_is_optional:
                ...
                globalComp.addShareToInvest('divestCosts_cancellation', self.owner, self.element_model.var_isInvested,
        ...
        if not (self.args.specificCosts is None):
            ...
            globalComp.addShareToInvest('specificCosts', self.owner, self.element_model.var_investmentSize,
