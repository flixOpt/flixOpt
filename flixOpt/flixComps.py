...
class LinearTransformer(Component):
    ...
    def __init__(self, label: str, inputs: list, outputs: list, exists=1, group: str = None, factor_Sets=None,
        ...
        **kwargs : TYPE
        ...
        if isinstance(exists, int) or (
            ...
            self.exists = TimeSeries('exists', exists, self)
        else:
        ...
        for flow in self.inputs + self.outputs:
            ...
            flow.exists = TimeSeries('exists', exists, flow)
            flow.max_rel = TimeSeries('max_rel', flow.max_rel.active_data * flow.exists.active_data, flow)
    ...
    def declareVarsAndEqs(self, modBox: SystemModel):
        ...
        :return:
        """
        super().declareVarsAndEqs(modBox)  # (ab hier sollte auch self.element_model.var_on dann vorhanden sein)
        ...
        if self.segmentsOfFlows is None:
            ...
            for i in range(len(self.factor_Sets)):
                ...
                for inFlow in leftSideFlows:
                    ...
                    eq_linearFlowRelation_i.addSummand(inFlow.element_model.var_val, aFactor)  # input1.val[t]      * factor[t]
                for outFlow in rightSideFlows:
                    aFactor = aFactorVec_Dict[outFlow].active_data
                    eq_linearFlowRelation_i.addSummand(outFlow.element_model.var_val, -aFactor)  # output.val[t] * -1 * factor[t]
...
class Storage(Component):
        ...
        **kwargs : TYPE # TODO welche kwargs werden hier genutzt???
        ...
        if isinstance(exists, int):
            ...
            if all(item in {0, 1} for item in exists):
                self.exists = TimeSeries('exists', np.append(exists, exists[-1]), self)
            else:
                raise ValueError("Invalid value for exists. Must contain only 0 and 1")

            self.max_rel_chargeState = TimeSeries('max_rel_chargeState',
                                                  self.max_rel_chargeState.active_data * self.exists.active_data, self)
            self.min_rel_chargeState = TimeSeries('min_rel_chargeState',
                                                  self.min_rel_chargeState.active_data * self.exists.active_data, self)
        else:
            raise TypeError()
    ...
    def declareVarsAndEqs(self, modBox: SystemModel):
        ...
        self.element_model.var_charge_state = VariableTS('charge_state', modBox.nrOfTimeSteps + 1, self, modBox, min=lb, max=ub,
                                                         value=fix_value)  # Eins mehr am Ende!
        self.element_model.var_charge_state.activateBeforeValues(self.chargeState0_inFlowHours, True)
        self.element_model.var_nettoFlow = VariableTS('nettoFlow', modBox.nrOfTimeSteps, self, modBox,
                                                      min=-np.inf)  # negative Werte zulässig!

        # erst hier, da definingVar vorher nicht belegt!
        if self.featureInvest is not None:
            self.featureInvest.setDefiningVar(self.element_model.var_charge_state, None)  # None, da kein On-Wert
    ...
    def doModeling(self, modBox, timeIndexe):
        ...
        if self.chargeState0_inFlowHours is None:
            ...
            self.eq_charge_state_start.addRightSide(self.element_model.var_charge_state.beforeVal())  # chargeState_0 !
            self.eq_charge_state_start.addSummand(self.element_model.var_charge_state, 1, timeIndexe[0])
        elif self.chargeState0_inFlowHours == 'lastValueOfSim':
            # eq: Q_Ladezustand(1) - Q_Ladezustand(end) = 0;
            self.eq_charge_state_start = Equation('charge_state_start', self, modBox, eqType='eq')
            self.eq_charge_state_start.addSummand(self.element_model.var_charge_state, 1, timeIndexe[0])
            self.eq_charge_state_start.addSummand(self.element_model.var_charge_state, -1, timeIndexe[-1])
        else:
        ...
        self.eq_charge_state.addSummand(self.element_model.var_charge_state,
        ...
        self.eq_charge_state.addSummand(self.element_model.var_charge_state, 1, timeIndexeChargeState[1:])  # 1:end
        self.eq_charge_state.addSummand(self.inFlow.element_model.var_val, -1 * self.eta_load.active_data * modBox.dtInHours)
        self.eq_charge_state.addSummand(self.outFlow.element_model.var_val,
        ...
        if self.charge_state_end_max is not None:
            ...
            self.eq_charge_state_end_max.addSummand(self.element_model.var_charge_state, 1, timeIndexeChargeState[-1])
        ...
        if self.charge_state_end_min is not None:
            ...
            self.eq_charge_state_end_min.addSummand(self.element_model.var_charge_state, -1, timeIndexeChargeState[-1])
        ...
        self.eq_nettoFlow.addSummand(self.element_model.var_nettoFlow, 1)
        self.eq_nettoFlow.addSummand(self.inFlow.element_model.var_val, 1)
        self.eq_nettoFlow.addSummand(self.outFlow.element_model.var_val, -1)
...
class cTransportation(Component):
    ...
    def doModeling(self, modBox, timeIndexe):
        ...
        self.eq_dir1.addSummand(self.in1.element_model.var_val, (1 - self.loss_rel.active_data))
        self.eq_dir1.addSummand(self.out1.element_model.var_val, -1)
        if (self.loss_abs.active_data is not None) and np.any(self.loss_abs.active_data != 0):
            assert self.in1.element_model.var_on is not None, 'Var on wird benötigt für in1! Set min_rel!'
            self.eq_dir1.addSummand(self.in1.element_model.var_on, -1 * self.loss_abs.active_data)

        # second direction:        
        if self.in2 is not None:
            # eq: in(t)*(1-loss_rel(t)) = out(t) + on(t)*loss_abs(t)
            self.eq_dir2 = Equation('transport_dir2', self, modBox, eqType='eq')
            self.eq_dir2.addSummand(self.in2.element_model.var_val, 1 - self.loss_rel.active_data)
            self.eq_dir2.addSummand(self.out2.element_model.var_val, -1)
            if (self.loss_abs.active_data is not None) and np.any(self.loss_abs.active_data != 0):
                assert self.in2.element_model.var_on is not None, 'Var on wird benötigt für in2! Set min_rel!'
                self.eq_dir2.addSummand(self.in2.element_model.var_on, -1 * self.loss_abs.active_data)
        ...
        if self.isAlwaysOn:
            ...
            self.eq_alwaysOn.addSummand(self.in1.element_model.var_on, -1)
            if (self.in2 is not None): self.eq_alwaysOn.addSummand(self.in2.element_model.var_on, -1)
        ...
        if (self.in2 is not None):
            ...
            if oneInFlowHasFeatureInvest:
                if bothInFlowsHaveFeatureInvest:
                    ...
                    self.eq_nom_value.addSummand(self.in2.featureInvest.element_model.var_investmentSize, -1)
                else:
                    raise Exception(
