...
class FlowModel(ElementModel):
    ...
    def relative_flow_rate_bounds(self) -> Tuple[Numeric, Numeric]:
        ...
        if fixed_profile is None:
            return (
                self.element.relative_minimum.active_data,
                self.element.relative_maximum.active_data,
            )
        return (
    ...
    def do_modeling(self, system_model: SystemModel) -> None:
        ...
        if self.element.with_excess:
            excess_penalty = np.multiply(
                ...
                self.element.excess_penalty_per_flow_hour.active_data,
            )
            ...
            fx_collection.add_share_to_penalty(
                f'{self.element.label_full}__excess_input',
                self.excess_input,
                excess_penalty,
            )
            fx_collection.add_share_to_penalty(
                f'{self.element.label_full}__excess_output',
                self.excess_output,
                excess_penalty,
            )
