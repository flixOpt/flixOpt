...
class SegmentedCalculation(Calculation):
    ...
    def _save_solve_infos(self):
        ...
        with open(self._paths['results'], 'w', encoding='utf-8') as f:
            results = copy_and_convert_datatypes(
                self.results(combined_arrays=True),
                use_numpy=False,
                use_element_label=False,
            )
            json.dump(results, f, indent=4)
        ...
        with open(
            self._paths['results'].parent / f'{self.name}_results_extra.json',
            'w',
            encoding='utf-8',
        ) as f:
            results = {
                'Individual Results': copy_and_convert_datatypes(
                    self.results(individual_results=True),
                    use_numpy=False,
                    use_element_label=False,
                ),
                'Skalar Results': copy_and_convert_datatypes(
                    self.results(combined_scalars=True),
                    use_numpy=False,
                    use_element_label=False,
                ),
            }
    ...
    def _get_indices(self, segment_index: int) -> range:
        ...
        return range(
            start,
            min(start + self.segment_length + self.overlap_length, self._total_length),
        )
...
def _combine_nested_scalars(
    *dicts: Dict[str, Union[Numeric, dict]],
) -> Dict[str, Union[List[Skalar], dict]]:
    """
