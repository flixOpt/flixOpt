...
def effect_values_from_effect_time_series(
    effect_time_series: EffectTimeSeries,
) -> Dict[Optional[Effect], Numeric]:
...
class EffectCollectionModel(ElementModel):
    ...
    def add_share_to_operation(
        ...
        self._add_share_to_effects(
            ...
            element,
            'operation',
            effect_values_from_effect_time_series(effect_values),
            factor,
            variable,
        )
    ...
    def add_share_between_effects(self):
        for origin_effect in self.element.effects.values():
            ...
            for (
                target_effect,
                time_series,
            ) in origin_effect.specific_share_to_other_effects_operation.items():
            ...
            for (
                target_effect,
                factor,
            ) in origin_effect.specific_share_to_other_effects_invest.items():
